...
const Slideshow = () => {
	...
	const nextForm = () => {
		setCurrentForm((currentForm) => (currentForm < 4 ? currentForm + 1 : 4));
	};

	const prevForm = () => {
		setCurrentForm((currentForm) => (currentForm > 1 ? currentForm - 1 : 1));
	};
	useEffect(() => {
		switch (currentForm) {
			case 1:
			...
			case 4:
				...
				if (formData[2] !== "" && formData[3] !== "") {
					...
					let saved = 3.11 * gallons - kWh * carData[formData[1]][1] * 0.16;
		...
		<>
			{currentForm !== 4 ? (
				...
				<>
					...
					<h1 className="text-white text-2xl font-bold px-2">Using EV {formData[1]}</h1>
				</>
			)}
			<button
